...
uri="http://java.sun.com/jsp/jstl/core" %> <%-- prefix : 접두사 uri(Uniform
Resource Identifier) : 자원을 구분하는 식별자 (주소 형태) --%>
...
<html lang="en">
    ...
    <body>
      ...
      3) scope : 내장 객체 범위 (기본값 : page)
        ...
        <%-- scriptlet 작성법 --%> <% pageContext.setAttribute("num1", 10); %>
        <%-- JSTL 작성법 --%>
      ...
      2) scope : 삭제할 내장 객체 범위(기본 값 : 모든 범위)
        ...
        * 주의 사항 * 
        ...
        <%-- sciptlet 작성법 --%> <% if(1==1) { %> 출력됩니다. <% } %> <%-- JSTL
        작성법 --%>
        <c:if test="${1==1}"> JSTL 작성법 </c:if>

        <%-- session 범위에 세팅된 test의 값이 "session"인 경우 --%> <%-- -
        문자열 비교 시 (==) 비교연산자 사용 가능 - 문자열임을 표기하는 리터럴 ==
        ''(홑따옴표) / "" (쌍따옴표) --%>
      ...
      c:otherwise : else를 나타내는 태그
        ...
        <c:choose>
            <c:when test="${temp2 > 100}"> 100보다 크다 </c:when>

            <c:when test="${temp2 < 100}"> 100보다 작다 </c:when>
        ...
        <table border="1">
            <c:forEach var="n" begin="1" end="10" varStatus="vs">
                <c:choose>
                    <%-- choose 내부에는 무조건 JSP 주석만 작성 --%> <%-- 첫번째
                    반복일 경우 --%>
